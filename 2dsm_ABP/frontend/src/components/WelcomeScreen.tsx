...
export default function WelcomeScreen({ onSelect }: WelcomeScreenProps) {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-gray-100">
      <header className="bg-white shadow-sm px-4 sm:px-6 py-3 flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="hidden sm:flex items-center border border-gray-300 rounded overflow-hidden max-w-full">
          <input
            className="text-sm outline-none px-3 py-1.5 w-36 sm:w-44 bg-transparent"
      ...
      <main className="flex flex-1 flex-col items-center justify-center px-6 py-12 gap-6">
        <div className="flex flex-col sm:flex-row items-center gap-3 text-center sm:text-left">
          <h1 className="text-2xl sm:text-3xl font-extrabold text-gray-900">Olá! eu sou o FAQtec</h1>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 mt-2 w-full max-w-md">
          {cards.map((card) => (
            <button
              key={card.type}

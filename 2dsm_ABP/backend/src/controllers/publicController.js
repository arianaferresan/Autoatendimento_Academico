import { getNavigationNodeService, deleteAllNodesService, createSupportContactService, } from "@/services/publicService.js";
import { createFulfillmentLogService } from "@/services/adminService.js";
export const getPublicNodes = async (req, res) => {
    // The ID can be '0', a number, or undefined if the route is /api/options/
    const { id } = req.params;
    const nodeId = !id || id === "0" ? null : parseInt(id, 10);
    if (id && id !== "0" && isNaN(nodeId)) {
...
export const createSupportContact = async (req, res) => {
    try {
        ...
        res.status(201).json({
            message: "Pergunta enviada com sucesso. Você receberá uma resposta no e-mail informado.",
        });
    }
    catch (error) {
...
export const createFulfillmentLog = async (req, res) => {
